...
fetch_file_exact() {
    ...
    if [ -z "$local_file" ]; then
        ...
        if [ -n "${WGET}" ]; then
            ...
            ${WGET} -t 1 -T 120 -q --no-check-certificate "${base_url}/${filename}"
        elif [ -n "${CURL}" ]; then
            log $DEBUG "Using curl for file fetch request: ${base_url}/${filename}"
            ${CURL} --connect-timeout 120 -f -s -k -O "${base_url}/${filename}" > /dev/null
        fi
...
fetch_file() {
    ...
    if [ -z "$local_file" ]; then
        ...
        if [ -n "${WGET}" ]; then
            file_list=`${WGET} -q --no-check-certificate -O - ${base_url}/ | tr '<' '\n' | grep HREF | sed -n -e '2,$s/A HREF=".*\/\([^"]*\).*/\1/p' | paste -s -d" " -`
        elif [ -n "${CURL}" ]; then
            file_list=`${CURL} -f -s -k --list-only ${base_url}/ | tr '<' '\n' | grep HREF | sed -n -e '2,$s/A HREF=".*\/\([^"]*\).*/\1/p' | paste -s -d" " -`
        fi
        ...
        if [ -n "${WGET}" ]; then
            log $DEBUG "Using wget for file fetch request: ${base_url}/${filename}"
            ${WGET} -t 1 -T 120 --no-check-certificate -q "${base_url}/${filename}"
        elif [ -n "${CURL}" ]; then
            log $DEBUG "Using curl for file fetch request: ${base_url}/${filename}"
            ${CURL} --connect-timeout 120 -f -s -k -O "${base_url}/${filename}" > /dev/null
        fi
...
test_connection() {
    ...
    if [ -n "${WGET}" ]; then
        ...
        ${WGET} -t 1 -T 120 -Sq -O- --no-check-certificate "${url}" > /dev/null 2>&1
    elif [ -n "${CURL}" ]; then
        log $DEBUG "Using curl for connection test: ${url}"
        ${CURL} --connect-timeout 120 -f -s -k "${url}" > /dev/null 2>&1
    fi
...
os_type_abbreviation() {
    ...
    if [ "${os_type}" = "linux" ]; then
        ...
        [ "${os_abr_vendor}" = "oracle" ] && eval ${abr}="ol"
    fi
...
verify_pkg() {
   ...
   pkg_file="$2"
   VP_RV=0
   VP_HDR="Header V4 RSA\(/SHA1\|/SHA256\|\) \(S\|s\)ignature"
   #VP_KEYOK="\(OK, \|\)key ID 5c344736\(\|: OK\)"
   VP_KEYOK="\(OK, \|\)key ID 67d941a1\(\|: OK\)"
   VP="V4 RSA\(/SHA1\|/SHA256\|\) \(S\|s\)ignature"
...
analyze_prerequisites_linux() {
    ...
    os_vendor=$2
    os_release=$3
    prerequisites_redhat_7="glibc:2.17 pam:1.1 libgcc:4.8 libxml2:2.9 zlib:1.2 openssl:1.0.1 libtool-ltdl util-linux"
    prerequisites_redhat_8="glibc:2.28 pam:1.3 libgcc libxml2:2.9 zlib:1.2 openssl:1.1.1 libtool-ltdl tar util-linux"
    prerequisites_redhat_9="glibc:2.34 pam:1.5 libgcc libxml2:2.9 zlib:1.2 openssl:3.0.1 libtool-ltdl tar util-linux"

    prerequisites_sles_12="glibc:2.19 pam libgcc_s1 libxml2-2 libz1 openssl:1.0.1 libtool util-linux"
    prerequisites_sles_15="glibc:2.26 pam libgcc_s1 libxml2-2 libz1 openssl:1.1 libtool util-linux"

    prerequisites_centos_7="${prerequisites_redhat_7}"

    prerequisites_oracle_7="${prerequisites_redhat_7}"
    prerequisites_oracle_8="${prerequisites_redhat_8}"
    prerequisites_oracle_9="${prerequisites_redhat_9}"

    prerequisites_raspbian_10="libpam-runtime openssl:1.1 libxml2:2.6 zlib1g:1:1.1.2 libltdl7 uuid-runtime"
    prerequisites_raspbian_11="libpam-runtime openssl:1.1 libxml2:2.9 zlib1g:1:1.2 libltdl7 uuid-runtime"

    prerequisites_ubuntu_18="libpam-runtime openssl:1.1 libxml2:2.6 zlib1g:1:1.1.2 libltdl7 uuid-runtime"
    prerequisites_ubuntu_20="libpam-runtime openssl:1.1 libxml2:2.6 zlib1g:1:1.1.2 libltdl7 uuid-runtime"
    prerequisites_ubuntu_22="libpam-runtime openssl:3.0.2 libxml2:2.9 zlib1g:1:1.1.2 libltdl7 uuid-runtime"
    ...
    if [ ${OS_VENDOR} = "debian" ]; then
        ...
        if [ ! -z "${query_release}" ]; then
            if [ -n "${package_release}" ]; then
                dpkg --compare-versions ${package_release} le ${query_release}
                if [ $? -eq 0 ]; then
                    return 0
                else
                    return 1
...
install_missing_prerequisites() {
    ...
    for pkg in ${missing}; do
        ...
        if [ "${os_type}" != "linux" ]; then
            ...
            fetch_file "https://${CORE_ADDR}/ldlogon/unix/${os_path}" "${filename}"
            if [ $? -ne 0 ]; then
...
install_agent_packages() {
    ...
    if [ $INI_INSTALL -ne 0 ] || [ $RPM_INSTALL -ne 0 -a -n "${CORE_ADDR}" ]; then
        ...
        for cert in ${CERT_FILES}; do
            fetch_file_exact "https://${CORE_ADDR}/ldlogon" "${cert}"
            [ $? -ne 0 ] && log $WARN "Failed to download ${cert} from https://${CORE_ADDR}/ldlogon"
        done
        ...
        for pkg in ${pkg_list}; do
            ...
            if [ $? -ne 0 ]; then
                ...
                fetch_file_exact "https://${CORE_ADDR}/ldlogon/unix/${os_path}" "${filename}"
                if [ $? -ne 0 ]; then
...
if [ -n "${INSTALL_PKGS}" ]; then
    ...
    if [ "${USE_CSA}" -eq 0 ]; then
	    ...
	    test_connection "https://${CORE_ADDR}"
	    [ $? -ne 0 ] && abort "Core connection test failed: https://${CORE_ADDR}" 1
	    log $INFO "Core connection test successful: https://${CORE_ADDR}"
    else
    	# Verify connection to CSA via IP or hostname
    	if [ -n "${CSA_IP}" ]; then
    		test_connection "https://${CSA_IP}"
    		[ $? -ne 0 ] && abort "CSA IP connection test failed: https://${CSA_IP}" 1
	    	log $INFO "CSA IP connection test successful: https://${CSA_IP}"
	    fi
	    if [ -n "${CSA_HOST}" ]; then
	    	test_connection "https://${CSA_HOST}"
    		[ $? -ne 0 ] && abort "CSA Host connection test failed: https://${CSA_HOST}" 1
	    	log $INFO "CSA IP connection test successful: https://${CSA_HOST}"
	    fi
    fi
